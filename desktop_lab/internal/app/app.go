...
func (a *App) initServices(repos *repository.Repository, wkhtmltopdfWindows []byte) *service.Services {
	svc := service.NewServices(
		repos.Material, repos.Standard, repos.Protocol, repos.Sample, repos.Group, repos.Token, repos.User, repos.Dimension, repos.Order, repos.Invitation, repos.Audit, repos.ProtocolVersion, repos.ProtocolTemplate, repos.Analytics,
...
func (a *App) initHandlers(svc *service.Services, frontendFS embed.FS) *handler.Handler {
	handl := handler.NewHandler(
		svc.Auth, svc.Dimensions, svc.Invitations, svc.Materials, svc.Groups, svc.Orders, svc.Profile,
